...
async function loadStreamCrypto(): Promise<StreamCryptoExports> {
    cryptoPromise ??= (async () => {
        const bytes = Buffer.from(VIDZEE_WASM_BASE64, 'base64');
        const wasm = (
        ...
        const { instance } = await wasm.instantiate(bytes, {
            env: {
                abort(
                    ...
                    throw new Error(
                        `VidZee decoder aborted at ${line}:${column} (${messagePointer}/${filePointer})`
                    );
                }
            }
        });
        return instance.exports as unknown as StreamCryptoExports;
    })();
    return cryptoPromise;
}

export type VidZeeDecodedStream = {
    language?: string;
    url: string;
    headers?: Record<string, string>;
};

export async function decryptVidZeeStream(
    encoded: string,
    hostname = 'player.vidzee.wtf'
): Promise<VidZeeDecodedStream | null> {
    if (!encoded || !hostname) return null;
    try {
        const exports = await loadStreamCrypto();
        const inputPointer = exports.__pin(
            allocateByteArray(exports, Buffer.from(encoded, 'base64'))
        );
        const hostnamePointer = allocateString(exports, hostname);
        try {
            const output = readByteArray(
                exports,
                exports.decrypt(inputPointer, hostnamePointer) >>> 0
            );
            if (!output?.length) return null;
            const parsed = JSON.parse(
                new TextDecoder().decode(output)
            ) as Partial<VidZeeDecodedStream>;
            if (
